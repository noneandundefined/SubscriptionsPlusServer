...
func Db(ctx context.Context, err error) *ServerError {
	...
	return InternalServerError("failed to perform database operation")
}
...
func Redis(ctx context.Context, err error) *ServerError {
	...
	if errors.Is(err, redis.Nil) {
		return NotFound(Err_RedisKeyNotFound.Error())
	}

	var netErr net.Error
	if errors.As(err, &netErr) {
		if netErr.Timeout() {
			return RequestTimeout(Err_RedisTimeout.Error())
		}

		return InternalServerError(Err_RedisNetwork.Error())
	}

	if errors.Is(err, context.DeadlineExceeded) {
		return RequestTimeout(Err_RedisDeadlineExceeded.Error())
	}

	if errors.Is(err, context.Canceled) {
		return Conflict(Err_RedisCanceled.Error())
	}

	return InternalServerError(Err_RedisOperationFailed.Error())
}
